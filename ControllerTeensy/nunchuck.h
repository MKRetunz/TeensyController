...
{      
  /*   Joy - X 0 137 255      [0]         
            Y 0 142 255      [1]   
            Acc - X 290  560  715  [2]  (*2*2)         
                Y 320  535  750  [3]  (*2*2)   
  */   // will use Joy X, Y for Coxa and femur, Acc Y for Tibia   
  /*   PosX = outbuf[1]/4;   
       PosZ = (outbuf[0]-128)/2;   
       PosY = (outbuf[2] - 72)/2;   
  */      
  NunC_JX = outbuf[0];   
  NunC_JY = outbuf[1];   
  NunC_AX = map(outbuf[2]*2*2, 256, 768, 0, 255);   
  NunC_AY = map(outbuf[3]*2*2, 256, 768, 0, 255);   
  NunC_AZ = map(outbuf[4]*2*2, 256, 768, 0, 255);   
  if ((outbuf[5] >> 0) & 1)  NunC_BZ = 0;
  else                       NunC_BZ = 1;
...
void NunC_SendNextByteRequest()
{
	...
	// otherwise only each 3rd call reads data from the controller (cnt will be 0 the other times)
        // Gidi: toch weer veranderd in 1, want dan reageert hij sneller 
	for(byte i = 0; i < 1; i++)
	{
		Wire.beginTransmission (WII_NUNCHUCK_TWI_ADR);	// transmit to device 0x52
...
boolean NunC_ReceiveCommand(){
	...
	for (cnt = 0; (cnt < WII_TELEGRAM_LEN) && Wire.available (); cnt++) {	
        ...
        clearTwiInputBuffer(); 	
        NunC_SendNextByteRequest();  // Gidi: added here in stead of in main sketch
